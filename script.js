...
document.querySelectorAll('.btn-recurso[download], a[download].btn-recurso').forEach(btn => {
  btn.addEventListener('click', function(e) {
    ...
    if (ebookUrl !== 'https://drive.google.com/uc?export=download&id=1WkcZuDH_Ar77-wvreRuacGZZuACjmnpc') {
...
// Tag dinamica radar
const radarTagEl = document.getElementById('radarTagText');
if (radarTagEl) {
  let ri = 0;
  setInterval(() => {
    radarTagEl.classList.add('fading');
    setTimeout(() => { ri=(ri+1)%phrases.length; radarTagEl.textContent=phrases[ri]; radarTagEl.classList.remove('fading'); }, 420);
  }, 3200);
}
