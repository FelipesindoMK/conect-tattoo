...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openContentPanel(targetId) {
    ...
    // A "migalha de pão" (breadcrumb) no topo é usada pela Wallet e pelo
    // Bazar — em qualquer outro painel, some.
    const contentCrumb = document.getElementById('contentCrumb');
    if (contentCrumb) contentCrumb.textContent = PANEL_CRUMB_LABELS[targetId] || '';
  ...
  // ==========================================================
  // WALLET — módulo completo (dashboard, depósito, tokens, assinatura,
  // extrato, métodos de pagamento, segurança). Estado ainda em memória
  // (sem persistência no Supabase) — é o próximo passo natural depois
  // que o visual estiver validado.
  // ==========================================================
  /* ---------------------------------------------------------
     ESTADO (mock, tudo em memória — sem backend real ainda)
  --------------------------------------------------------- */
  const walletState = {
    balance: 128.50,
    tokens: 340,
    ...
    planPrice: 0,
    cards: [
      { brand: 'VISA', last4: '4291', exp: '08/29', name: 'Ana Tatuadora', isDefault: true }
    ],
    transactions: [
      { type: 'deposito', label: 'Depósito via Pix', date: 'Hoje, 09:40', value: 100, dir: 'in' },
      { type: 'token', label: 'Conversão em tokens de IA', date: 'Ontem, 18:12', value: -50, dir: 'out' },
      { type: 'assinatura', label: 'Mensalidade — Plano Starter', date: '05/06/2026', value: 0, dir: 'out' },
      { type: 'pagamento', label: 'Compra — Manual do Tatuador (ebook)', date: '02/06/2026', value: -27, dir: 'out' },
      { type: 'deposito', label: 'Depósito via cartão', date: '28/05/2026', value: 200, dir: 'in' }
    ]
  };

  const WALLET_TX_ICONS = {
    deposito: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 4v16M6 14l6 6 6-6"/></svg>',
    token: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><ellipse cx="12" cy="12" rx="7" ry="7"/></svg>',
    assinatura: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14 3l3 6.2 6.8.9-5 4.7 1.3 6.8L14 18.3 7.9 21.6l1.3-6.8-5-4.7 6.8-.9z"/></svg>',
    pagamento: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="6" width="18" height="13" rx="2"/><path d="M3 10h18"/></svg>'
  };

  const WALLET_TX_LABELS = { deposito: 'Depósito', token: 'Conversão', assinatura: 'Assinatura', pagamento: 'Pagamento' };

  function walletFormatBRL(v) {
    const sign = v < 0 ? '-' : (v > 0 ? '+' : '');
    return sign + 'R$ ' + Math.abs(v).toFixed(2).replace('.', ',');
  }

  /* ---------------------------------------------------------
     NAVEGAÇÃO — usa o sistema de painéis que o site já tem
     (openContentPanel/closeContentOverlay, em script.js) — só
     acrescenta a "migalha de pão" (breadcrumb) no topo, que a Wallet e o
     Bazar usam.
  --------------------------------------------------------- */
  const PANEL_CRUMB_LABELS = {
    'panel-wallet-home': 'Wallet',
    'panel-wallet-deposit': 'Wallet › Depositar',
    'panel-wallet-tokens': 'Wallet › Tokens de IA',
    'panel-wallet-subscription': 'Wallet › Assinatura',
    'panel-wallet-extrato': 'Wallet › Extrato',
    'panel-wallet-methods': 'Wallet › Métodos de Pagamento',
    'panel-wallet-security': 'Wallet › Segurança',
    'panel-bazar-home': 'Desapego Bazar'
  };

  function walletShowPanel(id) {
    openContentPanel(id);
    if (id === 'panel-wallet-extrato') walletRenderFullExtrato('todos');
    if (id === 'panel-wallet-methods') walletRenderCardsList();
  }

  // O botão "Wallet" do widget de canto abria só um dropdown pequeno —
  // agora abre a tela cheia de verdade.
  const walletCornerBtn = document.getElementById('walletToggle');
  if (walletCornerBtn) {
    walletCornerBtn.addEventListener('click', () => {
      if (typeof closeAllCornerPanels === 'function') closeAllCornerPanels();
      walletRefreshHome();
      walletShowPanel('panel-wallet-home');
    });
  }

  document.querySelectorAll('[data-goto]').forEach(el => {
    el.addEventListener('click', () => walletShowPanel(el.dataset.goto));
  });

  document.querySelectorAll('[data-back-to]').forEach(btn => {
    btn.addEventListener('click', () => walletShowPanel(btn.dataset.backTo));
  });

  /* ---------------------------------------------------------
     MODAIS genéricos
  --------------------------------------------------------- */
  function walletOpenModal(id) { const m = document.getElementById(id); if (m) m.classList.add('open'); }
  function walletCloseModal(id) { const m = document.getElementById(id); if (m) m.classList.remove('open'); }
  document.querySelectorAll('[data-modal]').forEach(btn => {
    btn.addEventListener('click', () => walletOpenModal(btn.dataset.modal));
  });
  ...
  /* ---------------------------------------------------------
     RENDER — saldo, tokens, resumo do topo
  --------------------------------------------------------- */
  function walletRefreshBalances() {
    const balTxt = walletFormatBRL(walletState.balance).replace('+', '');
    const cornerBalanceEl = document.getElementById('cornerBalance');
    if (cornerBalanceEl) cornerBalanceEl.textContent = balTxt;
    const homeBalance = document.getElementById('homeBalance');
    if (homeBalance) homeBalance.textContent = balTxt;
    const homeTokens = document.getElementById('homeTokens');
    if (homeTokens) homeTokens.textContent = walletState.tokens;
    const tokensBalanceText = document.getElementById('tokensBalanceText');
    if (tokensBalanceText) tokensBalanceText.textContent = walletState.tokens + ' tokens';
    const tokensWalletBalance = document.getElementById('tokensWalletBalance');
    if (tokensWalletBalance) tokensWalletBalance.textContent = balTxt;
  }

  function walletRenderTxRow(tx) {
    return `
      <div class="wallet-tx-row">
        <div class="wallet-tx-icon ${tx.dir}">${WALLET_TX_ICONS[tx.type] || ''}</div>
        <div class="wallet-tx-info">
          <h4>${tx.label}</h4>
          <span>${tx.date}</span>
          <span class="wallet-tx-tag">${WALLET_TX_LABELS[tx.type] || tx.type}</span>
        </div>
        <div class="wallet-tx-value ${tx.dir}">${tx.value === 0 ? 'R$ 0,00' : walletFormatBRL(tx.value)}</div>
      </div>
    `;
  }

  function walletRefreshHome() {
    walletRefreshBalances();
    const preview = document.getElementById('homeTxPreview');
    if (preview) {
      const latest = walletState.transactions.slice(0, 3);
      preview.innerHTML = latest.length ? latest.map(walletRenderTxRow).join('') : '<p class="wallet-empty">Nenhuma movimentação ainda.</p>';
    }
    const homePlanName = document.getElementById('homePlanName');
    if (homePlanName) homePlanName.textContent = walletState.plan;
    const homeMethodsSummary = document.getElementById('homeMethodsSummary');
    if (homeMethodsSummary) homeMethodsSummary.textContent = `${walletState.cards.length} cartão(ões) · 1 chave Pix cadastrada`;
  }

  function walletAddTransaction(type, label, value, dir) {
    walletState.transactions.unshift({ type, label, date: 'Agora', value, dir });
  }

  /* ---------------------------------------------------------
     DEPÓSITO
  --------------------------------------------------------- */
  let walletDepositAmount = 100;
  const depositChips = document.getElementById('depositChips');
  const depositCustomValue = document.getElementById('depositCustomValue');

  if (depositChips) {
    depositChips.querySelectorAll('.wallet-amount-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        depositChips.querySelectorAll('.wallet-amount-chip').forEach(c => c.classList.remove('active'));
        chip.classList.add('active');
        walletDepositAmount = Number(chip.dataset.value);
        depositCustomValue.value = walletDepositAmount;
      });
    });
  }
  if (depositCustomValue) {
    depositCustomValue.addEventListener('input', () => {
      walletDepositAmount = Number(depositCustomValue.value) || 0;
      depositChips.querySelectorAll('.wallet-amount-chip').forEach(c => c.classList.remove('active'));
    });
  }

  const depositMethods = document.getElementById('depositMethods');
  if (depositMethods) {
    depositMethods.querySelectorAll('.wallet-method-card').forEach(card => {
      card.addEventListener('click', () => {
        depositMethods.querySelectorAll('.wallet-method-card').forEach(c => c.classList.remove('active'));
        card.classList.add('active');
        const method = card.dataset.method;
        document.querySelectorAll('.wallet-pay-panel').forEach(p => p.classList.toggle('active', p.dataset.payPanel === method));
      });
    });
  }

  function walletCompleteDeposit(method) {
    const value = walletDepositAmount > 0 ? walletDepositAmount : 100;
    walletState.balance += value;
    walletAddTransaction('deposito', `Depósito via ${method === 'pix' ? 'Pix' : method === 'boleto' ? 'boleto' : 'cartão de crédito'}`, value, 'in');
    walletRefreshBalances();
    const panel = document.querySelector(`.wallet-pay-panel[data-pay-panel="${method}"]`);
    if (panel) {
      panel.querySelector(`[data-pay-form="${method}"]`).style.display = 'none';
      panel.querySelector(`[data-success="${method}"]`).classList.add('show');
    }
    showToast(`Depósito de ${walletFormatBRL(value).replace('+','')} confirmado!`);
  }

  const copyPixBtn = document.getElementById('copyPixBtn');
  if (copyPixBtn) copyPixBtn.addEventListener('click', () => { showToast('Código Pix copiado!'); });
  const copyBoletoBtn = document.getElementById('copyBoletoBtn');
  if (copyBoletoBtn) copyBoletoBtn.addEventListener('click', () => { showToast('Código de barras copiado!'); });

  const confirmPixBtn = document.getElementById('confirmPixBtn');
  if (confirmPixBtn) confirmPixBtn.addEventListener('click', () => walletCompleteDeposit('pix'));
  const confirmBoletoBtn = document.getElementById('confirmBoletoBtn');
  if (confirmBoletoBtn) confirmBoletoBtn.addEventListener('click', () => walletCompleteDeposit('boleto'));
  const cardDepositForm = document.getElementById('cardDepositForm');
  if (cardDepositForm) cardDepositForm.addEventListener('submit', (e) => { e.preventDefault(); walletCompleteDeposit('cartao'); });

  /* ---------------------------------------------------------
     CONVERSÃO DE TOKENS
  --------------------------------------------------------- */
  const WALLET_TOKEN_RATE = 0.2; // R$ por token (referência: pacote Bronze)
  let walletPendingConversion = null;

  document.querySelectorAll('.wallet-token-card[data-pack]').forEach(card => {
    const pack = card.dataset.pack;
    if (pack === 'custom') return;
    const btn = card.querySelector('button');
    btn.addEventListener('click', () => {
      const tokens = Number(card.dataset.tokens);
      const price = Number(card.dataset.price);
      walletOpenTokenConfirm(tokens, price);
    });
  });
  ...
  if (customTokenBtn) {
    customTokenBtn.addEventListener('click', () => {
      ...
      if (!tokens || tokens <= 0) { showToast('Digite uma quantidade válida de tokens.'); return; }
      const price = Math.round(tokens * WALLET_TOKEN_RATE * 100) / 100;
      walletOpenTokenConfirm(tokens, price);
    });
  }

  function walletOpenTokenConfirm(tokens, price) {
    walletPendingConversion = { tokens, price };
    document.getElementById('confirmTokensText').textContent = `Converter R$ ${price.toFixed(2).replace('.', ',')} do seu saldo em ${tokens} tokens de IA.`;
    walletOpenModal('modalConfirmTokens');
  }

  const confirmTokensBtn = document.getElementById('confirmTokensBtn');
  if (confirmTokensBtn) {
    confirmTokensBtn.addEventListener('click', () => {
      if (!walletPendingConversion) return;
      const { tokens, price } = walletPendingConversion;
      if (walletState.balance < price) {
        walletCloseModal('modalConfirmTokens');
        showToast('Saldo insuficiente — deposite antes de converter.');
        pushPanel('panel-wallet-deposit');
        return;
      }
      walletState.balance -= price;
      walletState.tokens += tokens;
      walletAddTransaction('token', `Conversão em ${tokens} tokens de IA`, -price, 'out');
      walletRefreshBalances();
      walletCloseModal('modalConfirmTokens');
      showToast(`${tokens} tokens adicionados à sua carteira!`);
      walletPendingConversion = null;
    });
  }
  ...
  /* ---------------------------------------------------------
     ASSINATURA
  --------------------------------------------------------- */
  let walletPendingPlan = null;

  document.querySelectorAll('[data-choose-plan]').forEach(btn => {
    btn.addEventListener('click', () => {
      const card = btn.closest('.wallet-plan-card');
      const plan = card.dataset.plan;
      const price = Number(card.dataset.price);
      walletPendingPlan = { plan, price };
      document.getElementById('confirmPlanText').textContent = `Assinar o plano ${plan} por R$ ${price.toFixed(2).replace('.', ',')}/mês.`;
      walletOpenModal('modalConfirmPlan');
    });
  });
  ...
  if (confirmPlanBtn) {
    confirmPlanBtn.addEventListener('click', () => {
      if (!walletPendingPlan) return;
      const { plan, price } = walletPendingPlan;
      walletState.plan = plan;
      walletState.planPrice = price;
      walletAddTransaction('assinatura', `Mensalidade — Plano ${plan}`, -price, 'out');
      walletRefreshHome();

      document.querySelectorAll('.wallet-plan-card').forEach(c => {
        const isNowCurrent = c.dataset.plan === plan;
        c.classList.toggle('current', isNowCurrent);
        c.querySelector('.wallet-plan-badge-current')?.remove();
        const btn2 = c.querySelector('button');
        if (isNowCurrent) {
          if (!c.querySelector('.wallet-plan-badge-current')) {
            const badge = document.createElement('span');
            badge.className = 'wallet-plan-badge-current';
            badge.textContent = 'Atual';
            c.insertBefore(badge, c.firstChild);
          }
          btn2.disabled = true; btn2.textContent = 'Plano atual'; btn2.classList.remove('btn-primary'); btn2.classList.add('btn-outline');
        } else {
          btn2.disabled = false; btn2.classList.remove('btn-outline'); btn2.classList.add('btn-primary');
          btn2.textContent = 'Assinar ' + c.dataset.plan;
        }
      });
      document.getElementById('subCurrentPlanName').textContent = plan;
      document.getElementById('subNextValue').textContent = price.toFixed(2).replace('.', ',');

      walletCloseModal('modalConfirmPlan');
      showToast(`Plano ${plan} ativado com sucesso!`);
      walletPendingPlan = null;
    });
  }
  ...
  /* ---------------------------------------------------------
     EXTRATO
  --------------------------------------------------------- */
  const fullTxList = document.getElementById('fullTxList');
  function walletRenderFullExtrato(filter) {
    if (!fullTxList) return;
    const list = filter === 'todos' ? walletState.transactions : walletState.transactions.filter(t => t.type === filter);
    fullTxList.innerHTML = list.length ? list.map(walletRenderTxRow).join('') : '<p class="wallet-empty">Nenhuma movimentação nessa categoria ainda.</p>';
  }
  const txFilters = document.getElementById('txFilters');
  if (txFilters) {
    txFilters.querySelectorAll('.rp-pill').forEach(pill => {
      pill.addEventListener('click', () => {
        txFilters.querySelectorAll('.rp-pill').forEach(p => p.classList.remove('active'));
        pill.classList.add('active');
        walletRenderFullExtrato(pill.dataset.filter);
      });
    });
  }
  ...
  function walletRenderCardsList() {
    if (!cardsList) return;
    cardsList.innerHTML = walletState.cards.map((c, idx) => `
      <div class="wallet-card-row" data-card-idx="${idx}">
        <div class="wallet-card-brand">${c.brand}</div>
        <div class="wallet-card-info"><h4>•••• •••• •••• ${c.last4}</h4><span>${c.name} · válido até ${c.exp}</span></div>
        ${c.isDefault ? '<span class="wallet-card-default">Padrão</span>' : '<button class="btn btn-outline btn-sm" data-set-default>Definir padrão</button>'}
        <button class="wallet-card-remove" data-remove-card><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 3l10 10M13 3L3 13"/></svg></button>
      </div>
    `).join('');

    cardsList.querySelectorAll('[data-remove-card]').forEach(btn => {
      btn.addEventListener('click', () => {
        const row = btn.closest('.wallet-card-row');
        const idx = Number(row.dataset.cardIdx);
        walletState.cards.splice(idx, 1);
        walletRenderCardsList();
        walletRefreshHome();
        showToast('Cartão removido.');
      });
    });
    cardsList.querySelectorAll('[data-set-default]').forEach(btn => {
      btn.addEventListener('click', () => {
        const row = btn.closest('.wallet-card-row');
        const idx = Number(row.dataset.cardIdx);
        walletState.cards.forEach((c, i) => c.isDefault = (i === idx));
        walletRenderCardsList();
        showToast('Cartão definido como padrão.');
      });
    });
  }
  ...
  if (addCardForm) {
    addCardForm.addEventListener('submit', (e) => {
      ...
      const number = inputs[0].value.replace(/\s/g, '');
      const exp = inputs[1].value || '00/00';
      const name = inputs[3].value || 'Titular';
      walletState.cards.push({ brand: 'CARD', last4: number.slice(-4) || '0000', exp, name, isDefault: walletState.cards.length === 0 });
      walletRenderCardsList();
      walletRefreshHome();
      walletCloseModal('modalAddCard');
      addCardForm.reset();
      showToast('Cartão adicionado com sucesso!');
    });
  }

  const addPixForm = document.getElementById('addPixForm');
  if (addPixForm) {
    addPixForm.addEventListener('submit', (e) => {
      e.preventDefault();
      walletCloseModal('modalAddPix');
      addPixForm.reset();
  ...
  if (saveDailyLimitBtn) {
    saveDailyLimitBtn.addEventListener('click', () => {
      ...
      showToast(`Limite diário atualizado para R$ ${Number(val).toFixed(2).replace('.', ',')}`);
    });
  }

  /* ---------------------------------------------------------
     INICIALIZAÇÃO — Wallet
  --------------------------------------------------------- */
  walletRefreshBalances();


  // ==========================================================
  // DESAPEGO BAZAR — módulo completo (busca, categorias, feed,
  // detalhe do anúncio, publicar novo item). Estado ainda em
  // memória (sem persistência no Supabase), mesmo estágio que a
  // Wallet por enquanto.
  // ==========================================================

  /* ---------------------------------------------------------
     CATEGORIAS — rótulos e ícone (mesmo ícone usado no grid)
  --------------------------------------------------------- */
  const BAZ_CATS = {
    maquinas:      { label: 'Máquinas de Tatuagem',           icon: '<path d="M6 18l6-12 6 12"/><path d="M9 12h6"/>' },
    fontes:        { label: 'Fontes & Power Supply',          icon: '<path d="M13 2L4 14h6l-1 8 9-12h-6z"/>' },
    macas:         { label: 'Macas',                          icon: '<path d="M3 17h18M4 17V9a2 2 0 012-2h6l4 4h4a2 2 0 012 2v4"/>' },
    cadeiras:      { label: 'Cadeiras',                       icon: '<path d="M6 4v10M18 4v16M6 14h12M6 20l-2 0M18 20h2"/>' },
    impressoras:   { label: 'Impressoras de Stencil',         icon: '<path d="M6 9V3h12v6"/><rect x="4" y="9" width="16" height="8" rx="1"/><path d="M6 17v4h12v-4"/>' },
    cartuchos:     { label: 'Cartuchos de Impressora',        icon: '<rect x="7" y="3" width="10" height="18" rx="2"/><path d="M10 3V1h4v2"/>' },
    esterilizacao: { label: 'Autoclaves & Estufas',           icon: '<rect x="4" y="7" width="16" height="13" rx="2"/><path d="M8 7V5a4 4 0 018 0v2"/>' },
    mobiliario:    { label: 'Mobiliário',                     icon: '<path d="M4 21V8a2 2 0 012-2h12a2 2 0 012 2v13"/><path d="M4 14h16"/>' },
    iluminacao:    { label: 'Iluminação',                     icon: '<path d="M9 18h6M10 21h4"/><path d="M12 3a6 6 0 00-3 11c.6.5 1 1.3 1 2h4c0-.7.4-1.5 1-2a6 6 0 00-3-11z"/>' },
    espelhos:      { label: 'Espelhos',                       icon: '<rect x="6" y="3" width="12" height="18" rx="6"/>' },
    climatizacao:  { label: 'Ar-condicionado',                icon: '<rect x="3" y="6" width="18" height="6" rx="1"/><path d="M6 12v6M10 12v6M14 12v6M18 12v6"/>' },
    digital:       { label: 'Tablets & Mesas Digitalizadoras', icon: '<rect x="4" y="3" width="16" height="18" rx="2"/><path d="M9 21h6"/>' }
  };

  /* ---------------------------------------------------------
     ESTADO — anúncios mock (em memória, sem backend real)
  --------------------------------------------------------- */
  let bazListings = [
    { id: 1, cat: 'macas', title: 'Maca elétrica hidráulica, pouco uso', price: 1350, cond: 'Seminovo', desc: 'Comprei há 8 meses, uso 2x por semana. Só desapegando porque fechei o estúdio físico e vou atender só a domicílio.', seller: 'Rafael Costa', loc: 'Presidente Epitácio, SP', time: 'há 2h', neg: true, fav: false, mine: false },
    { id: 2, cat: 'maquinas', title: 'Máquina rotativa profissional + case', price: 480, cond: 'Usado', desc: 'Kit com 2 máquinas rotativas e case de transporte. Toda revisada, funcionando perfeitamente.', seller: 'Camila Duarte', loc: 'Presidente Prudente, SP', time: 'há 5h', neg: true, fav: false, mine: false },
    { id: 3, cat: 'cadeiras', title: 'Cadeira reclinável de cliente (couro sintético)', price: 890, cond: 'Seminovo', desc: 'Cadeira confortável, poucos riscos no acabamento, ideal pra sessões longas.', seller: 'Larissa Prado', loc: 'Bauru, SP', time: 'há 1 dia', neg: false, fav: false, mine: false },
    { id: 4, cat: 'esterilizacao', title: 'Autoclave 12L com selo Inmetro', price: 2200, cond: 'Usado', desc: 'Autoclave revisada recentemente, manual em mãos, nota fiscal disponível.', seller: 'Marcos Teles', loc: 'Assis, SP', time: 'há 1 dia', neg: true, fav: false, mine: false },
    { id: 5, cat: 'impressoras', title: 'Impressora térmica de stencil', price: 620, cond: 'Muito usado', desc: 'Funciona bem, tem alguns riscos na carcaça. Ideal pra quem tá começando e quer economizar.', seller: 'Bruna Lopes', loc: 'Presidente Epitácio, SP', time: 'há 2 dias', neg: true, fav: false, mine: false },
    { id: 6, cat: 'fontes', title: 'Fonte de alimentação digital dual', price: 340, cond: 'Seminovo', desc: 'Fonte com display digital, alimenta 2 máquinas ao mesmo tempo. Pouquíssimo uso.', seller: 'Rafael Costa', loc: 'Presidente Epitácio, SP', time: 'há 3 dias', neg: false, fav: false, mine: false },
    { id: 7, cat: 'mobiliario', title: 'Balcão de recepção + armário organizador', price: 750, cond: 'Usado', desc: 'Conjunto completo pra recepção de estúdio, madeira maciça, bom estado.', seller: 'Camila Duarte', loc: 'Presidente Prudente, SP', time: 'há 4 dias', neg: true, fav: false, mine: false },
    { id: 8, cat: 'digital', title: 'Tablet + caneta digitalizadora pra desenho', price: 1600, cond: 'Seminovo', desc: 'Uso só pra criar referências e stencils digitais. Vem com película e capa.', seller: 'Marcos Teles', loc: 'Assis, SP', time: 'há 5 dias', neg: true, fav: false, mine: false },
    { id: 9, cat: 'iluminacao', title: 'Luminária de mesa articulada com lupa', price: 220, cond: 'Usado', desc: 'Ótima pra detalhes finos, braço articulado longo, lâmpada LED inclusa.', seller: 'Larissa Prado', loc: 'Bauru, SP', time: 'há 6 dias', neg: false, fav: false, mine: false },
    { id: 10, cat: 'cartuchos', title: 'Kit de cartuchos de tinta pra impressora de stencil (lacrados)', price: 90, cond: 'Seminovo', desc: 'Sobraram desses da minha impressora anterior, ainda lacrados na caixa.', seller: 'Bruna Lopes', loc: 'Presidente Epitácio, SP', time: 'há 1 semana', neg: false, fav: false, mine: false }
  ];
  let bazNextId = 11;

  let bazActiveCat = 'todos';
  let bazActiveTab = 'feed';
  let bazSearchTerm = '';
  let bazActiveSort = 'recentes';

  /* ---------------------------------------------------------
     NAVEGAÇÃO — usa o sistema de painéis que o site já tem
     (openContentPanel, em script.js) — mesmo padrão da Wallet.
  --------------------------------------------------------- */
  function bazOpenBazar() {
    openContentPanel('panel-bazar-home');
    bazRenderFeed();
  }

  const bazarCornerBtn = document.getElementById('bazarToggle');
  if (bazarCornerBtn) {
    bazarCornerBtn.addEventListener('click', () => {
      if (typeof closeAllCornerPanels === 'function') closeAllCornerPanels();
      bazOpenBazar();
    });
  }
  ...
  /* ---------------------------------------------------------
     MODAIS genéricos
  --------------------------------------------------------- */
  function bazOpenModal(id) { const m = document.getElementById(id); if (m) m.classList.add('open'); }
  function bazCloseModal(id) { const m = document.getElementById(id); if (m) m.classList.remove('open'); }
  document.querySelectorAll('[data-modal]').forEach(btn => {
    btn.addEventListener('click', () => bazOpenModal(btn.dataset.modal));
  });
  document.querySelectorAll('.rp-modal-overlay').forEach(overlay => {
    overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.classList.remove('open'); });
    overlay.querySelectorAll('[data-close-modal]').forEach(btn => {
  ...
  /* ---------------------------------------------------------
     CATEGORIAS
  --------------------------------------------------------- */
  const bazCatGrid = document.getElementById('bazCatGrid');
  if (bazCatGrid) {
    bazCatGrid.querySelectorAll('.baz-cat-card').forEach(card => {
      card.addEventListener('click', () => {
        bazCatGrid.querySelectorAll('.baz-cat-card').forEach(c => c.classList.remove('active'));
        card.classList.add('active');
        bazActiveCat = card.dataset.cat;
        bazRenderFeed();
      });
    });
  }
  ...
  /* ---------------------------------------------------------
     BUSCA
  --------------------------------------------------------- */
  const bazSearchInput = document.getElementById('bazSearchInput');
  const bazSearchBtn = document.getElementById('bazSearchBtn');
  function bazRunSearch() { bazSearchTerm = (bazSearchInput.value || '').trim().toLowerCase(); bazRenderFeed(); }
  if (bazSearchBtn) bazSearchBtn.addEventListener('click', bazRunSearch);
  if (bazSearchInput) {
    bazSearchInput.addEventListener('input', bazRunSearch);
    bazSearchInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') bazRunSearch(); });
  }

  /* ---------------------------------------------------------
     TABS — Feed / Meus anúncios / Favoritos
  --------------------------------------------------------- */
  const bazTabs = document.getElementById('bazTabs');
  if (bazTabs) {
    bazTabs.querySelectorAll('.baz-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        bazTabs.querySelectorAll('.baz-tab').forEach(t => t.classList.remove('active'));
        tab.classList.add('active');
        bazActiveTab = tab.dataset.tab;
        bazRenderFeed();
      });
    });
  }
  ...
  /* ---------------------------------------------------------
     ORDENAÇÃO
  --------------------------------------------------------- */
  const bazSort = document.getElementById('bazSort');
  if (bazSort) bazSort.addEventListener('change', () => { bazActiveSort = bazSort.value; bazRenderFeed(); });

  /* ---------------------------------------------------------
     RENDER DO FEED
  --------------------------------------------------------- */
  const bazFeed = document.getElementById('bazFeed');
  const bazEmptyMsg = document.getElementById('bazEmptyMsg');

  function bazGetFilteredListings() {
    let list = bazListings.slice();

    if (bazActiveTab === 'meus') list = list.filter(l => l.mine);
    else if (bazActiveTab === 'favoritos') list = list.filter(l => l.fav);

    if (bazActiveCat !== 'todos') list = list.filter(l => l.cat === bazActiveCat);

    if (bazSearchTerm) list = list.filter(l => l.title.toLowerCase().includes(bazSearchTerm) || (BAZ_CATS[l.cat] && BAZ_CATS[l.cat].label.toLowerCase().includes(bazSearchTerm)));

    if (bazActiveSort === 'menor-preco') list.sort((a, b) => a.price - b.price);
    else if (bazActiveSort === 'maior-preco') list.sort((a, b) => b.price - a.price);
    // 'recentes' e 'perto' mantêm a ordem mock atual (mais recentes primeiro / simulação de proximidade)

    return list;
  }

  function bazInitials(name) { return name.trim().charAt(0).toUpperCase(); }

  function bazRenderPostCard(item) {
    const catInfo = BAZ_CATS[item.cat] || { label: item.cat, icon: '<circle cx="12" cy="12" r="9"/>' };
    return `
      <div class="baz-post-card" data-id="${item.id}">
        <div class="baz-post-head">
          <div class="baz-avatar">${bazInitials(item.seller)}</div>
          <div class="baz-post-seller">
            <h4>${item.mine ? 'Você' : item.seller}</h4>
            <span>${item.loc} · ${item.time}</span>
          </div>
        </div>
        <div class="baz-post-photo"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4">${catInfo.icon}</svg></div>
        <div class="baz-post-body">
          <span class="eyebrow" style="margin-bottom:0;">${catInfo.label}</span>
          <h3>${item.title}</h3>
          <div class="baz-post-price-row">
            <span class="baz-post-price">R$ ${item.price.toLocaleString('pt-BR')}</span>
            <span class="baz-post-cond">${item.cond}</span>
          </div>
          ${item.neg ? '<span class="baz-post-negotiable">Aceita proposta</span>' : ''}
          <p class="baz-post-desc">${item.desc}</p>
          <div class="baz-post-actions">
            <button class="btn btn-outline btn-sm baz-fav-btn ${item.fav ? 'active' : ''}" data-fav="${item.id}">
              <svg viewBox="0 0 24 24" fill="${item.fav ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="1.8"><path d="M12 21s-7.5-4.6-10-9.2C.6 8.3 2 4.5 5.6 3.6c2.1-.5 4.2.5 5.4 2.3 1.2-1.8 3.3-2.8 5.4-2.3 3.6.9 5 4.7 3.6 8.2C19.5 16.4 12 21 12 21z"/></svg>
            </button>
            <button class="btn btn-primary" data-detail="${item.id}">Ver detalhes</button>
          </div>
        </div>
      </div>
    `;
  ...
  function bazRenderFeed() {
    if (!bazFeed) return;
    const list = bazGetFilteredListings();
    if (list.length === 0) {
      bazFeed.innerHTML = '';
      bazEmptyMsg.style.display = 'block';
      return;
    }
    bazEmptyMsg.style.display = 'none';
    bazFeed.innerHTML = list.map(bazRenderPostCard).join('');

    bazFeed.querySelectorAll('[data-fav]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const id = Number(btn.dataset.fav);
        const item = bazListings.find(l => l.id === id);
        if (item) {
          item.fav = !item.fav;
          showToast(item.fav ? 'Adicionado aos favoritos' : 'Removido dos favoritos');
          bazRenderFeed();
        }
      });
    });
    bazFeed.querySelectorAll('[data-detail]').forEach(btn => {
      btn.addEventListener('click', () => bazOpenDetail(Number(btn.dataset.detail)));
    });
  }
  ...
  /* ---------------------------------------------------------
     MODAL DE DETALHE
  --------------------------------------------------------- */
  function bazOpenDetail(id) {
    const item = bazListings.find(l => l.id === id);
    if (!item) return;
    const catInfo = BAZ_CATS[item.cat] || { label: item.cat, icon: '<circle cx="12" cy="12" r="9"/>' };

    document.getElementById('detailCatLabel').textContent = catInfo.label;
    document.getElementById('detailTitle').textContent = item.title;
    document.getElementById('detailPhoto').innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4">${catInfo.icon}</svg>`;
    document.getElementById('detailPrice').textContent = 'R$ ' + item.price.toLocaleString('pt-BR');
    document.getElementById('detailCond').textContent = item.cond;
    document.getElementById('detailDesc').textContent = item.desc;
    document.getElementById('detailAvatar').textContent = bazInitials(item.seller);
    document.getElementById('detailSellerName').textContent = item.mine ? 'Você' : item.seller;
    document.getElementById('detailSellerMeta').textContent = `${item.loc} · publicado ${item.time}`;

    const offerRow = document.querySelector('.baz-offer-row');
    const offerInput = document.getElementById('detailOfferInput');
    offerInput.value = '';
    offerRow.style.display = item.neg ? 'flex' : 'none';

    const chatBtn = document.getElementById('detailChatBtn');
    chatBtn.onclick = () => { showToast('Abrindo conversa com ' + item.seller + ' no chat da LogOS...'); };
    document.getElementById('detailOfferBtn').onclick = () => {
      const val = offerInput.value.trim();
      if (!val) { showToast('Digite um valor pra propor.'); return; }
      showToast(`Proposta de ${val} enviada pra ${item.seller}!`);
      bazCloseModal('modalDetail');
    };

    bazOpenModal('modalDetail');
  }
  ...
  if (anunciarForm) {
    anunciarForm.addEventListener('submit', (e) => {
      e.preventDefault();
      const cat = document.getElementById('anCategoria').value;
      const title = document.getElementById('anTitulo').value.trim();
      const price = Number(document.getElementById('anPreco').value) || 0;
      const cond = document.getElementById('anCondicao').value;
      const desc = document.getElementById('anDescricao').value.trim() || 'Sem descrição adicional.';
      const loc = document.getElementById('anLocal').value.trim() || 'Localização não informada';
      const neg = document.getElementById('anAceitaProposta').checked;

      bazListings.unshift({
        id: bazNextId++, cat, title, price, cond, desc, seller: 'Você', loc, time: 'agora mesmo', neg, fav: false, mine: true
      });

      bazCloseModal('modalAnunciar');
      anunciarForm.reset();
      showToast('Anúncio publicado no Bazar!');

      // Leva o tatuador direto pra aba "Meus anúncios" pra ele ver o resultado
      bazTabs.querySelectorAll('.baz-tab').forEach(t => t.classList.toggle('active', t.dataset.tab === 'meus'));
      bazActiveTab = 'meus';
      bazRenderFeed();
    });
  }
  ...
  // (Os antigos botões walletDepositBtn/walletConvertBtn do mockup foram
  // substituídos pelas telas reais da Wallet — panel-wallet-deposit e
  // panel-wallet-tokens, abertas via data-goto.)
